// xlsx-lite.ts — lector mínimo de .xlsx (zip+xml) y CSV/TSV para tablas.
// Ported from project/xlsx-lite.js.
...
export async function extractPDFText(buf: ArrayBuffer): Promise<string> {
  ...
  for (let i = 0; i < u8.length; i += CH)
    bin += String.fromCharCode.apply(null, Array.from(u8.subarray(i, Math.min(u8.length, i + CH))))
  ...
  while ((m = re.exec(bin))) {
    ...
    const data = bin.slice(start, end).replace(/\r?\n$/, '')
    ...
    if (/FlateDecode/.test(d)) {
      try {
        ...
        for (let i = 0; i < out.length; i += CH)
          sx += String.fromCharCode.apply(
            null,
            Array.from(out.subarray(i, Math.min(out.length, i + CH))),
          )
        text = sx
      } catch (e) {}
  ...
  const dec = (t: string) =>
    t
      ...
      .replace(/\\(\d{1,3})/g, (_q, o) => String.fromCharCode(parseInt(o, 8)))
  const lines: string[] = []
  for (const content of parts) {
    const tokens =
      content.match(/\((?:\\.|[^\\()])*\)\s*Tj|\[(?:\\.|[^\]\\])*\]\s*TJ|T\*|TD|Td|ET/g) || []
    let line = ''
