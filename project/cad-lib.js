// cad-lib.js — Parser DXF (ASCII), generador SVG y plano de ejemplo.
// Entidades internas:
...
export const UNIT_MM = { m: 1000, cm: 10, mm: 1 };

function cleanText(t) {
  return String(t)
    .replace(/%%[dD]/g, '°').replace(/%%[cC]/g, 'Ø').replace(/%%[pP]/g, '±').replace(/%%%/g, '%');
}
...
function isIdent(m) { return m.a === 1 && m.b === 0 && m.c === 0 && m.d === 1 && m.e === 0 && m.f === 0; }

function arcToPts(cx, cy, r, a1, a2) {
  let s = a1 * Math.PI / 180, t = a2 * Math.PI / 180;
...
function transformEnt(e, m) {
  ...
  switch (e.k) {
    case 'l': {
      const p1 = apply(m, e.x1, e.y1), p2 = apply(m, e.x2, e.y2);
    ...
    case 'a': {
      // bajo transformación general, muestrear a polilínea (robusto ante espejos/escala no uniforme)
...
export function parseDXF(text) {
  ...
  const get = (seq, code) => { for (const p of seq) if (p[0] === code) return p[1]; };
  const getAll = (seq, code) => { const r = []; for (const p of seq) if (p[0] === code) r.push(p[1]); return r; };

  function collect(start) {
    const type = String(pairs[start][1]).trim();
    let j = start + 1;
    const seq = [];
    while (j < pairs.length && pairs[j][0] !== 0) { seq.push(pairs[j]); j++; }
    return { type, seq, end: j };
  }

  function attachStyle(b, seq) {
    const cv = get(seq, 62);
    if (cv !== undefined) { const ci = parseInt(cv, 10); if (ci > 0 && ci < 256) b.col = ci; }
    const tv = get(seq, 420);
    if (tv !== undefined) { const t = parseInt(tv, 10); if (isFinite(t) && t >= 0) b.tc = t; }
    const lv = get(seq, 370);
    if (lv !== undefined) { const lw = parseInt(lv, 10); if (lw > 0) b.lw = lw; }
  }

  function buildEntity(type, seq) {
    const layer = String(get(seq, 8) || '0').trim();
    if (/^defpoints$/i.test(layer)) return null;
    const f = c => parseFloat(get(seq, c));
    switch (type) {
      case 'LINE': {
      ...
      case 'LWPOLYLINE': {
        ...
        for (const [c, v] of seq) {
          if (c === 10) { cur = [parseFloat(v), 0, 0]; pts.push(cur); }
      ...
      case 'ELLIPSE': {
        const cx = f(10), cy = f(20), mx = f(11), my = f(21), ratio = f(40);
        ...
        for (let q = 0; q <= n; q++) {
          ...
          const px = Math.cos(u) * L, py = Math.sin(u) * L * ratio;
          pts.push([cx + px * Math.cos(ang) - py * Math.sin(ang), cy + px * Math.sin(ang) + py * Math.cos(ang), 0]);
        }
        return { k: 'p', layer, pts, closed: Math.abs((u2 - u1) - Math.PI * 2) < 1e-6 };
      }
      case 'SPLINE': {
        const cx0 = getAll(seq, 10).map(parseFloat), cy0 = getAll(seq, 20).map(parseFloat);
        const fx = getAll(seq, 11).map(parseFloat), fy = getAll(seq, 21).map(parseFloat);
        const px = fx.length > 1 ? fx : cx0, py = fy.length > 1 ? fy : cy0;
        if (px.length < 2) return null;
        const pts = px.map((x, q) => [x, py[q], 0]).filter(p => isFinite(p[0]) && isFinite(p[1]));
      ...
      case 'TEXT': case 'ATTRIB': {
        ...
        if (!raw) return null;
        let x = f(10), y = f(20);
        const ha = parseInt(get(seq, 72) || '0', 10), va = parseInt(get(seq, 73) || '0', 10);
        if ((ha || va) && isFinite(f(11))) { x = f(11); y = f(21); }
      ...
      case 'MTEXT': {
        ...
        const x = f(10), y = f(20);
        if (![x, y].every(isFinite)) return null;
        return { k: 't', layer, x, y, h: f(40) || 1, rot: f(50) || 0, text: txt, mtext: true, att: parseInt(get(seq, 71) || '1', 10) };
      }
      case 'POINT': {
        const x = f(10), y = f(20);
      ...
      case 'SOLID': case '3DFACE': {
        const q = [[f(10), f(20)], [f(11), f(21)], [f(13), f(23)], [f(12), f(22)]].filter(p => p.every(isFinite));
        return q.length > 2 ? { k: 'p', layer, pts: q.map(p => [p[0], p[1], 0]), closed: true, fill: type === 'SOLID' } : null;
      }
      case 'INSERT': {
        const name = String(get(seq, 2) || '').trim();
        if (!name) return null;
        const sx = isFinite(f(41)) ? f(41) : 1;
        return { k: 'i', layer, name, x: f(10) || 0, y: f(20) || 0, sx, sy: isFinite(f(42)) ? f(42) : sx, rot: f(50) || 0 };
      }
      case 'DIMENSION': {
        const bn = String(get(seq, 2) || '').trim();
        // grupo 12/22: desplazamiento para cotas clonadas (baseline/continue)
        const ox = isFinite(f(12)) ? f(12) : 0, oy = isFinite(f(22)) ? f(22) : 0;
        return bn ? { k: 'i', layer, name: bn, x: ox, y: oy, sx: 1, sy: 1, rot: 0 } : null;
      }
      case 'MULTILEADER': case 'MLEADER': {
        // anotación con directriz: texto en grupos 304 (dentro de CONTEXT_DATA),
        // posición del texto en 12/22 (o base 10/20), altura en 41/140.
        // el grupo 304 se usa también como marcador de estructura «LEADER_LINE{»:
        // solo los valores que no son marcadores forman el texto
        let txt = getAll(seq, 304).filter(v => !/^LEADER_LINE\{$/.test(String(v).trim())).join('');
        txt = cleanMText(txt.replace(/\^J/g, '\n'));
        if (!txt.trim()) return null;
        let x = f(12), y = f(22);
        if (![x, y].every(isFinite)) { x = f(10); y = f(20); }
        if (![x, y].every(isFinite)) return null;
        const h41 = f(41), h140 = f(140);
        const h = (isFinite(h41) && h41 > 0) ? h41 : (isFinite(h140) && h140 > 0 ? h140 : 1);
        const out = [{ k: 't', layer, x, y, h, rot: 0, text: txt, mtext: true, att: 1 }];
        // geometría real de la directriz: vértices de LEADER_LINE{…}, último
        // punto de LEADER{…} y codo final (dogleg = dirección 11/21 × longitud 40)
        const pts = [];
        let mode = '', px = null, lx = null, lastPt = null, dgx = null, dgy = null, dgLen = null;
        for (const [c, v] of seq) {
          const sv = String(v).trim();
          if (c === 302 && sv === 'LEADER{') { mode = 'leader'; continue; }
          if (c === 304 && sv === 'LEADER_LINE{') { mode = 'line'; px = null; continue; }
          if (c === 305 && sv === '}') { mode = 'leader'; px = null; continue; }
          if (c === 303 && sv === '}') { mode = ''; continue; }
          if (mode === 'line') {
            if (c === 10) px = parseFloat(v);
            else if (c === 20 && px !== null && isFinite(px) && isFinite(parseFloat(v))) { pts.push([px, parseFloat(v), 0]); px = null; }
          } else if (mode === 'leader') {
            if (c === 10) lx = parseFloat(v);
            else if (c === 20 && lx !== null && isFinite(lx) && isFinite(parseFloat(v))) { lastPt = [lx, parseFloat(v)]; lx = null; }
        ...
        if (lastPt && isFinite(dgx) && isFinite(dgy) && isFinite(dgLen) && dgLen > 0) {
  ...
  function readPolyline(first) {
    // first = collect() de POLYLINE; sigue VERTEX... hasta SEQEND
    const closed = (parseInt(get(first.seq, 70) || '0', 10) & 1) === 1;
    const layer = String(get(first.seq, 8) || '0').trim();
    let i2 = first.end;
    const pts = [];
    while (i2 < pairs.length) {
      if (pairs[i2][0] !== 0) { i2++; continue; }
      const t = String(pairs[i2][1]).trim();
      if (t === 'VERTEX') {
        const v = collect(i2);
        const x = parseFloat(get(v.seq, 10)), y = parseFloat(get(v.seq, 20));
        const b = parseFloat(get(v.seq, 42)) || 0;
        if (isFinite(x) && isFinite(y)) pts.push([x, y, b]);
        i2 = v.end;
      } else if (t === 'SEQEND') {
        const s2 = collect(i2); i2 = s2.end; break;
      } else break;
    }
    return { ent: pts.length > 1 ? (() => { const ent = { k: 'p', layer, pts, closed }; attachStyle(ent, first.seq); return ent; })() : null, end: i2 };
  }
    ...
    if (c === 0 && String(v).trim() === 'SECTION') {
      ...
      if (name === 'HEADER') {
        while (i < pairs.length && !(pairs[i][0] === 0 && String(pairs[i][1]).trim() === 'ENDSEC')) {
          if (pairs[i][0] === 9 && String(pairs[i][1]).trim() === '$INSUNITS' && pairs[i + 1] && pairs[i + 1][0] === 70) {
        ...
        while (i < pairs.length && !(pairs[i][0] === 0 && String(pairs[i][1]).trim() === 'ENDSEC')) {
          if (pairs[i][0] === 0 && String(pairs[i][1]).trim() === 'BLOCK') {
            ...
            const bx = parseFloat(get(h.seq, 10)) || 0, by = parseFloat(get(h.seq, 20)) || 0;
            i = h.end;
            const bents = [];
            while (i < pairs.length && !(pairs[i][0] === 0 && String(pairs[i][1]).trim() === 'ENDBLK')) {
              ...
              if (e.type === 'POLYLINE') {
                ...
                if (Array.isArray(b)) b.forEach(bb => { attachStyle(bb, e.seq); bents.push(bb); });
                else if (b) { attachStyle(b, e.seq); bents.push(b); }
                i = e.end;
              }
            }
        ...
        while (i < pairs.length && !(pairs[i][0] === 0 && String(pairs[i][1]).trim() === 'ENDSEC')) {
          ...
          if (e.type === 'POLYLINE') {
            ...
            const b = buildEntity(e.type, e.seq);
            if (Array.isArray(b)) b.forEach(bb => { attachStyle(bb, e.seq); rawEnts.push(bb); });
            else if (b) { attachStyle(b, e.seq); rawEnts.push(b); }
            i = e.end;
          }
        }
      }
  ...
  const layers = [...new Set(flat.map(e => e.layer).filter(Boolean))];
  const texts = flat.filter(e => e.k === 't').slice(0, 400).map(e => ({ t: e.text, x: e.x, y: e.y, h: e.h }));
  const dim = Math.max(bounds.w, bounds.h);
  const unitsGuess = ({ 4: 'mm', 5: 'cm', 6: 'm' })[units] || (dim > 2000 ? 'mm' : dim > 100 ? 'cm' : 'm');
  return { ents: flat, bounds, layers, texts, unitsGuess, n: flat.length, layerStyles: layerTable };
}

// ---- bounds ----
export function computeBounds(ents) {
  ...
  for (const e of ents) {
    if (e.k === 'l') { ext(e.x1, e.y1); ext(e.x2, e.y2); }
    else if (e.k === 'p') { for (const p of e.pts) ext(p[0], p[1]); }
    else if (e.k === 'c' || e.k === 'a') { ext(e.cx - e.r, e.cy - e.r); ext(e.cx + e.r, e.cy + e.r); }
    else if (e.k === 't') { ext(e.x, e.y); ext(e.x + String(e.text || ' ').length * e.h * 0.62, e.y + e.h * 1.3); }
...
function entCenter(e) {
  ...
  if (e.k === 'p') { let sx = 0, sy = 0; for (const p of e.pts) { sx += p[0]; sy += p[1]; } return [sx / e.pts.length, sy / e.pts.length]; }
...
function rectOf(e) {
  // rectángulo cerrado axis-aligned de 4 vértices (5 si repite el primero), sin bulges
  if (e.k !== 'p' || !e.closed) return null;
  const pts = e.pts;
  if (pts.length < 4 || pts.length > 5) return null;
  if (pts.some(p => p[2])) return null;
  const xs = pts.map(p => p[0]), ys = pts.map(p => p[1]);
  const minX = Math.min(...xs), maxX = Math.max(...xs), minY = Math.min(...ys), maxY = Math.max(...ys);
  const w = maxX - minX, h = maxY - minY;
  if (w <= 0 || h <= 0) return null;
  const tol = Math.max(w, h) * 0.002;
  const onCorner = pts.every(p =>
...
export function detectFrames(model, unit, frameTest) {
  const um = UNIT_MM[unit] || 1000;
  const SIZES = { A4: [210, 297], A3: [297, 420], A2: [420, 594], A1: [594, 841] };
  ...
  for (const e of model.ents) {
    ...
    outer: for (const size of Object.keys(SIZES)) {
      ...
      for (const N of ESC) {
        const pw = r.w * um / N, ph = r.h * um / N;
        if (Math.abs(pw - dims[0]) < dims[0] * 0.02 && Math.abs(ph - dims[1]) < dims[1] * 0.02) { best = { size, orient: 'p', escala: N }; break outer; }
        if (Math.abs(pw - dims[1]) < dims[1] * 0.02 && Math.abs(ph - dims[0]) < dims[0] * 0.02) { best = { size, orient: 'l', escala: N }; break outer; }
      }
    }
    if (!best) continue;
    let inside = 0;
    for (const o of model.ents) {
      if (o === e) continue;
      const c = entCenter(o);
      if (c && c[0] > r.minX && c[0] < r.maxX && c[1] > r.minY && c[1] < r.maxY) inside++;
    }
  ...
  // deduplicar marcos casi idénticos
  cands.sort((a, b) => (a.minX - b.minX) || (a.minY - b.minY));
  const out = [];
  for (const c of cands) {
    if (!out.some(o => Math.abs(o.minX - c.minX) < c.w * 0.02 && Math.abs(o.minY - c.minY) < c.h * 0.02 && Math.abs(o.w - c.w) < c.w * 0.02)) out.push(c);
  }
  return out;
}

// ---- SVG ----
const ACI_EXACT = { 1: '#FF0000', 2: '#E0B000', 3: '#00A650', 4: '#00B0C8', 5: '#0000FF', 6: '#FF00FF', 7: '#17161A', 8: '#808080', 9: '#C0C0C0', 250: '#545454', 251: '#767676', 252: '#989898', 253: '#BABABA', 254: '#DCDCDC', 255: '#17161A' };
function aciToHex(c) {
...
function tcToHex(t) { return '#' + (t & 0xFFFFFF).toString(16).padStart(6, '0').toUpperCase(); }
function darkenIfPale(hex) {
...
let _clipSeq = 0;
export function buildSVG(ents, bounds, opts = {}) {
  ...
  const isFrameRect = (e) => {
    ...
    const r = rectOf(e);
    return !!r && Math.abs(r.minX - b.minX) < frameTol && Math.abs(r.maxX - b.maxX) < frameTol &&
      Math.abs(r.minY - b.minY) < frameTol && Math.abs(r.maxY - b.maxY) < frameTol;
  };
  ...
  for (const e of ents) {
    ...
    // estilo por capa/entidad (colores y grosores del DXF)
    let eCol = null, eSw = null;
    if (opts.useLayers) {
    ...
    if (e.k === 'l') body += `<line x1="${F(e.x1)}" y1="${F(e.y1)}" x2="${F(e.x2)}" y2="${F(e.y2)}"${sa}/>`;
    else if (e.k === 'c') body += `<circle cx="${F(e.cx)}" cy="${F(e.cy)}" r="${F(e.r)}"${sa}/>`;
    else if (e.k === 'a') {
      const s0 = e.a1 * Math.PI / 180;
      const x1 = e.cx + e.r * Math.cos(s0), y1 = e.cy + e.r * Math.sin(s0);
      const t0 = e.a2 * Math.PI / 180;
      const x2 = e.cx + e.r * Math.cos(t0), y2 = e.cy + e.r * Math.sin(t0);
      let sweep = (e.a2 - e.a1) % 360; if (sweep <= 0) sweep += 360;
      ...
      const anchor = e.mtext
        ? ([2, 5, 8].includes(e.att) ? 'middle' : [3, 6, 9].includes(e.att) ? 'end' : 'start')
        : (e.ha === 1 || e.ha === 4 ? 'middle' : e.ha === 2 ? 'end' : 'start');
      const fs = e.h * 1.35;
      // anclaje vertical del MTEXT según attachment (1-3 arriba, 4-6 centro, 7-9 abajo)
      const dy0 = e.mtext ? ([4, 5, 6].includes(e.att) ? e.h * 0.4 : [7, 8, 9].includes(e.att) ? -e.h * 0.25 : e.h * 1.05) : 0;
      let tsp = '';
      linesT.forEach((ln, ix) => {
        tsp += `<tspan x="0" dy="${ix === 0 ? F(dy0) : F(e.h * 1.5)}">${esc(ln)}</tspan>`;
      });
      body += `<text transform="translate(${F(e.x)} ${F(e.y)}) rotate(${F(e.rot || 0)}) scale(1,-1)" font-size="${F(fs)}" font-family="${font}" font-weight="500" letter-spacing="${F(e.h * 0.06)}" text-anchor="${anchor}" fill="${fcol}" stroke="none">${tsp}</text>`;
    } else if (e.k === 'pt') {
      body += `<circle cx="${F(e.x)}" cy="${F(e.y)}" r="${F(sw * 1.5)}" fill="${fcol}" stroke="none"/>`;
    }
  }
  const inner = `<g transform="scale(1,-1)" fill="none" stroke="${color}" stroke-width="${F(sw)}" stroke-linecap="round" stroke-linejoin="round">${body}</g>`;
  if (region) {
    const cid = 'gcclip' + (++_clipSeq);
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${vb}" width="100%" height="100%" preserveAspectRatio="xMidYMid meet"><defs><clipPath id="${cid}"><rect x="${F(b.minX)}" y="${F(-b.maxY)}" width="${F(b.w)}" height="${F(b.h)}"/></clipPath></defs><g clip-path="url(#${cid})">${inner}</g></svg>`;
  }
...
export function sampleModel() {
  const E = [];
  const L = (layer, x1, y1, x2, y2) => E.push({ k: 'l', layer, x1, y1, x2, y2 });
  const R = (layer, x1, y1, x2, y2) => E.push({ k: 'p', layer, closed: true, pts: [[x1, y1, 0], [x2, y1, 0], [x2, y2, 0], [x1, y2, 0]] });
  const C = (layer, cx, cy, r) => E.push({ k: 'c', layer, cx, cy, r });
  const A = (layer, cx, cy, r, a1, a2) => E.push({ k: 'a', layer, cx, cy, r, a1, a2 });
  const T = (layer, x, y, h, text, rot) => E.push({ k: 't', layer, x, y, h, rot: rot || 0, text });

  // MUROS del stand (trasera + laterales, grosor 0,10) — frente abierto en y=0
  R('MUROS', 0, 2.9, 6, 3.0);         // pared trasera
  R('MUROS', 0, 0, 0.1, 2.9);         // lateral izquierdo
  R('MUROS', 5.9, 0, 6.0, 2.9);       // lateral derecho

  // TARIMA — borde frontal
  L('TARIMA', 0, 0, 6, 0);

  // TABIQUES — almacén trasero derecho con puerta
  R('TABIQUES', 4.6, 1.6, 4.7, 2.9);
  R('TABIQUES', 4.7, 1.6, 5.0, 1.7);
  R('TABIQUES', 5.7, 1.6, 5.9, 1.7);
  // puerta del almacén (hoja + barrido)
  L('CARPINTERIA', 5.0, 1.6, 5.0, 0.9);
  A('CARPINTERIA', 5.0, 1.6, 0.7, 270, 360);

  // MOBILIARIO
  R('MOBILIARIO', 0.4, 0.2, 1.9, 0.7);                       // mostrador
  C('MOBILIARIO', 3.2, 1.5, 0.4);                            // mesa alta
  C('MOBILIARIO', 2.6, 1.15, 0.15); C('MOBILIARIO', 3.8, 1.15, 0.15); C('MOBILIARIO', 3.2, 2.15, 0.15); // taburetes
  R('MOBILIARIO', 2.2, 2.75, 3.4, 2.9);                      // pantalla en trasera
  R('MOBILIARIO', 0.1, 1.2, 0.45, 2.6);                      // expositor lateral
  R('MOBILIARIO', 4.75, 1.75, 5.35, 2.85);                   // estantería almacén
  ...
  // COTAS generales
  L('COTAS', 0, -0.5, 6, -0.5); L('COTAS', 0, -0.6, 0, -0.4); L('COTAS', 6, -0.6, 6, -0.4);
  T('COTAS', 2.83, -0.42, 0.14, '6,00');
  L('COTAS', 6.5, 0, 6.5, 3); L('COTAS', 6.4, 0, 6.6, 0); L('COTAS', 6.4, 3, 6.6, 3);
